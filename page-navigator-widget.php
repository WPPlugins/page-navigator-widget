...
class PageNavigatorWidget extends WP_Widget {
	...
	function pnw_get_pages($root_id = 0, $sortby = 'post_title', $addhome = 'never') {
		...
		switch ( $sortby ) {
			case 'post_title':
				$sort = 'post_title, menu_order';
				break;
			case 'menu_order':
				$sort = 'menu_order, post_title';
				break;
			default:
				$sort = $sortby;
		}
		$query = "SELECT ID, post_title FROM `" . $wpdb->posts . "` WHERE `post_type` = 'page' AND `post_status` = 'publish' AND `post_parent` = " . $root_id . " ORDER BY " . $sort;
		$pages = $wpdb->get_results( $query, ARRAY_A );
		if ( $pages ) {
			$widget_content .= '<ul style="margin-bottom:0;" >';
			if ( $root_id == 0 ) {
